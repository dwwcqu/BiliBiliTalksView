...
版本 2.0.0 固定后不得原地改变其契约；修改须先说明影响并取得用户确认，再发布新版本。Schema 和实现与本文冲突时应报告并修正实现，不静默修改标准。实现细节如数据库表名、语言和模块路径不属于本协议。后续模型分析需另行约定输出结构，不得把分析标签直接混入本协议的原始评论记录。

## 与历史版本的兼容

本版根据已确认的英文稳定路径需求，从 [历史协议 1.0.0](comment-export-protocol-v1.md) 升为 2.0.0。仅改变生成路径和 schema_version：各元数据、评论记录及发布入口的 schema_version 统一为 `2.0.0`；其他 JSON 字段、类型、身份关系、覆盖判定和正文语义与 1.0.0 相同。`batches/<export_id>/` 结构及 UUID 格式不变。

新的 CLI 默认生成 2.0.0；读取、校验和数据库导入继续兼容 1.0.0，按明确版本选择对应路径契约，不把两版规则混用在一个批次。将旧批次转换为本版时，新建 export_id、重建两种分类及索引，保留原始正文、昵称、采集区间和覆盖状态，不仅重命名目录或修改单个 manifest。旧 1.0.0 文件的技术定义保持不变。

迁移不得覆盖或删除用户修改过的旧目录；新输出可以选择独立 output 根，由其 current.json 指向本版批次。不得把保留的用户文件自动认作当前权威批次。当前部署输出根由运维指定，路径示例不是生产硬编码默认值。
...
用户目录固定为 `users/uid_<UID>/`，例如 `users/uid_12345/`；未知作者目录为 `users/unknown/`。目录名不包含昵称，昵称变化不改变用户目录。display_nickname 仍取该批次同 UID 最新 collected_at 的非空昵称，同刻按评论 ID 数值顺序取最后一条；无可得昵称时为 null。原始昵称和正文仅存入 JSON 字段，不清洗、不替换、不删改。

自动生成的目录段只能匹配 `^[a-z0-9_-]+$`，不能包含中文、空格、换行、控制字符或其他符号。批次内路径固定为 `threads/<至少三位序号>/thread.json`、对应 `comments.jsonl`，以及 `users/uid_<UID>/user.json` 或 `users/unknown/user.json`、对应 `comments.jsonl`。固定文件名为 `README.md`、`manifest.json`、`thread.json`、`user.json`、`comments.jsonl`、可选 `unclassified.jsonl`；文件扩展名的点和 README 大写不属于目录段限制，发布入口仍名为 `current.json`。

用户通过输出参数指定的根目录支持 Windows/Linux 路径，该根目录本身不受内部目录段规则限制；程序不得擅自重命名用户目录。元数据中的路径使用相对 POSIX 分隔符，禁止绝对路径和 ..，读写时校验不逃逸导出根目录。
...
回复父 ID 若在当前楼中缺失，仍保留原值并登记缺口；不改接根评论。跨楼父引用属于异常并报告。来源未提供直接回复目标时，不从平铺顺序猜测。未知 UID 的评论仍保留在楼文件，统一放入 `users/unknown/`，user.json 的 uid 为 null、identity_status 为 unknown；该目录是未能归属的集合，不能视作一个用户。
...
- counts：`{root_comments, replies, comments, known_users, unknown_author_comments, unclassified_records}`，均为非负整数；comments = root_comments + replies，只计合规唯一记录，known_users 不含 unknown。
- source_reported_count：来源最近一次报告的视频总评论数或 null，仅供参考，不能证明覆盖。
- threads：`[{root_id, path}]`；path 指向 thread.json。
- users：`[{uid, path}]`；path 指向 user.json，unknown 项 uid 为 null。
...
每个视频的固定发布入口为 `<output>/bilibili-video-<aid>/current.json`；output 为调用方指定的输出根，例如 `D:/Data/exports` 或 `/data/exports`。其父目录是发布容器，批次目录 `batches/<export_id>/` 才是上文包含 README、manifest 和两个分类目录的“视频目录”。所有 manifest、thread、user 内的相对路径都以该批次目录为根；不得相对于 current.json 解析。
...
同一视频只保留当前导出和最近一次临时工作数据，不建立历史归档。采集中断可导出明确 partial 的批次供检查，但不自动替换已有 verified 批次。数据保存在被 Git 忽略的 `data/exports/`，正式写入前检查忽略规则。仓库只保存设计、对应实现、Schema 与脱敏测试样例。
...
- known 身份必须有 uid；unknown 身份必须 uid=null。unknown 目录只在存在未知作者评论时生成，绝不能作为一个真实用户参与统计。
...
> 请严格遵循《BiliBiliTalksView 评论导出协议》2.0.0。使用 UID 关联用户、root_id 关联楼、parent_id 关联直接回复；所有外部数字 ID 使用字符串。按楼与按用户的文件必须使用同一批次的相同评论记录。读取时同时检查分页覆盖与上下文缺口，遵守 current.json 的整批读取规则。不得自行变更字段、目录语义或枚举；发现冲突先报告并提出版本化修改建议。当前任务只按另行批准的实施计划执行，不因持有本协议而自动开展采集、模型调用或部署。
